...
def api_order_location(order_id: str):
    ...
    # Hard-stop tracking for delivered orders
    strow = db_fetchone(f"SELECT status FROM orders WHERE id={_ph()}", (order_id,))
    if strow and (strow.get("status") == "DELIVERED"):
        return jsonify({"ok": False, "error": "tracking_closed"}), 403
...
def chat_order(order_id: str):
    ...
    strow = db_fetchone(f"SELECT status FROM orders WHERE id={_ph()}", (order_id,))
    order_status = (strow.get("status") if strow else None) or ""
    chat_closed = (order_status == "DELIVERED")
...
def api_order_send_message(order_id: str):
    ...
    # Hard-stop chat on delivered orders
    strow = db_fetchone(f"SELECT status FROM orders WHERE id={_ph()}", (order_id,))
    if strow and (strow.get("status") == "DELIVERED"):
        return jsonify({"ok": False, "error": "chat_closed"}), 403
